...
class MqttNachricht(object):
    ...
    def __init__(self, pattern, name, timeout):
        ...
        mqtt_sup.register(self)
        for i in DEBUG_KLASSE:
            if i == self.__class__.__name__:
                log_sup.log().debug("MqttNachricht,  {name:16s}, neues Pattern {pattern:s}".format(
                    name=self.m_name,
    ...
    def print_alles(self):
        ...
        for i in DEBUG_KLASSE:
            if i == self.__class__.__name__:
                self.m_lock.acquire()
                log_sup.log().debug("MqttNachricht, {n:16s}, t: {z:8s}, w: {w:8s}, {g:3s}".format(
                    n=self.m_name,
    ...
    def subscribe(self):
        ...
        if not mqtt_sup.subscribe(self.m_pattern): # _mqttclient
            log_sup.log().error("MqttNachricht,  {name:16s}, Fehler bei subscribe to {s:s}".format(
                name=self.m_name,
                s=self.m_pattern))
        else:
            log_sup.log().debug("MqttNachricht,  {name:16s}, subscribed to {s:s}".format(
                name=self.m_name,
                s=self.m_pattern))
...
class MqttSensor(MqttNachricht):
    ...
    def gueltig(self):
        if MqttNachricht.gueltig(self):
            return True
        log_sup.log().debug("MqttSensor, {name:16s}, Noch kein gueltiger Wert, warte auf einen".format(
            name=self.m_name))
        self.m_gueltig_event.wait(5) # sollte noch kein Wert da sein, max. 5 Sekunden warten
        if MqttNachricht.gueltig(self):
            log_sup.log().debug("MqttSensor, {name:16s}, Jetzt gueltiger Wert".format(
                name=self.m_name))
            return True
        log_sup.log().warning("MqttSensor, {name:16s}, Immer noch kein gueltiger Wert".format(
            name=self.m_name))
        return False
...
class MqttTemperatur(MqttSensor):
    ...
    def temperatur_check(self):
        ...
        log_sup.log().debug("In neuem Thread")
        if not _Status_Helligkeit.gueltig():
            log_sup.log().warning("MqttTemperatur, {name:16s}, Kein Helligkeitswert".format(
                name=self.m_name))
            return
        helligkeit = int(_Status_Helligkeit.wert())
        if helligkeit < 3000:
            log_sup.log().debug("MqttTemperatur, {name:16s}, zu dunkel h: {hell:d}".format(
                name=self.m_name,
                hell=helligkeit))
            return
        if _Shelly_SZ.automatic_mode():
            if _Shelly_SZ.position_oben():
                ...
                log_sup.log().debug("MqttTemperatur, {name:16s}, Shelly Rollo nicht oben".format(
                    name=self.m_name))
        else:
            log_sup.log().debug("MqttTemperatur, {name:16s}, Shelly nicht im Auto-Mode".format(
                name=self.m_name))
    ...
    def update(self, pattern, nachricht):
        ...
        if MqttNachricht.update(self, pattern, nachricht):
            ...
            zeit = time.localtime()
            stunde = zeit.tm_hour
            log_sup.log().debug("MqttTemperatur, {name:16s}, T: {t:f} stunde: {s:d} L_A: {a:f}".format(
            ...
            if (temperatur > 27.0) and (stunde > 10) and (stunde < 20) and (aktiv_timeout > 3600):
                log_sup.log().debug("MqttTemperatur, {name:16s}, Temperatur und Zeit passt".format(
                    name=self.m_name))
                log_sup.log().debug("Starte neuen Thread")
...
class MqttHelligkeit(MqttSensor):
    ...
    def helligkeit_check(self):
        ...
        if _Shelly_SZ.automatic_mode():
            if _Shelly_SZ.position_oben():
                ...
                log_sup.log().debug("MqttHelligkeit, {name:16s}, Shelly Rollo nicht oben".format(
                    name=self.m_name))
        else:
            log_sup.log().debug("MqttHelligkeit, {name:16s}, Shelly nicht im Auto-Modus".format(
                name=self.m_name))
    ...
    def update(self, pattern, nachricht):
        ...
        if MqttNachricht.update(self, pattern, nachricht):
            ...
            zeit = time.localtime()
            stunde = zeit.tm_hour
            log_sup.log().debug("MqttHelligkeit, {name:16s}, H: {h:d} stunde: {s:d} L_A: {a:f}".format(
            ...
            if ((helligkeit > 50) and (helligkeit < 500) and (stunde > 3) and (stunde < 6) and
                    (aktiv_timeout > 3600)):
                log_sup.log().debug("MqttHelligkeit, {n:16s}, Helligkeitsbereich und Zeit passt".format(
                    n=self.m_name))
                log_sup.log().debug("Starte neuen Thread")
...
class MqttShelly(MqttNachricht):
    ...
    def gueltig(self):
        if MqttNachricht.gueltig(self):
            return True
        log_sup.log().debug("MqttShelly, {name:16s}, Noch kein gueltiger Wert, fordere neuen an".format(
            name=self.m_name))
        self.m_shelly.trigger_mqtt()
        self.m_gueltig_event.wait(5) # sollte noch kein Wert da sein, max. 5 Sekunden warten
        if MqttNachricht.gueltig(self):
            log_sup.log().debug("MqttShelly, {name:16s}, Jetzt gueltiger Wert".format(
                name=self.m_name))
            return True
        log_sup.log().warning("MqttShelly, {name:16s}, Immer noch kein gueltiger Wert".format(
            name=self.m_name))
        return False
...
class Shelly(object):
    ...
    def print_alles(self):
        ...
        for i in DEBUG_KLASSE:
            if i == self.__class__.__name__:
                log_sup.log().debug("Shelly, {name:16s}, Basis-Pattern {wert:s}".format(
                    name=self.m_name,
                    wert=self.m_pattern))
                return
    ...
    def trigger_mqtt(self):
        ...
        In der n�chsten Schleife sollten dann gueltige Werte da sein.
        """
        log_sup.log().debug("Shelly, {name:16s}, trigger_mqtt".format(name=self.m_name))
        try:
            urllib2.urlopen("http://"+self.m_ip + "/settings?mqtt_update_period=2")
        except urllib2.HTTPError, fehler:
            log_sup.log().error("Shelly, {name:16s}, trigger_mqtt 1 HTTPError".format(name=self.m_name))
            log_sup.log().error(fehler)
        except urllib2.URLError, fehler:
            log_sup.log().error("Shelly, {name:16s}, trigger_mqtt 1 URLError".format(name=self.m_name))
            log_sup.log().error(fehler)
#        else:
        for i in range(3):
            del i
            ...
            try:
                ...
                log_sup.log().error("Shelly, {name:16s}, trigger_mqtt 2 HTTPError".format(name=self.m_name))
                log_sup.log().error(fehler)
            except urllib2.URLError, fehler:
                log_sup.log().error("Shelly, {name:16s}, trigger_mqtt 2 URLError".format(name=self.m_name))
                log_sup.log().error(fehler)

    def automatic_mode(self): # pylint : disable=too-many-return-statements
        ...
        if not self.m_schalter_0.gueltig():
            log_sup.log().warning("Shelly, {name:16s}, Schalter(0) kein aktueller Wert".format(
                name=self.m_name))
            return False
        if self.m_schalter_0.wert() != "0":
            log_sup.log().debug("Shelly, {name:16s}, Schalter(0) nicht aus, Wert: {wert:s}".format(
                name=self.m_name, wert=self.m_schalter_0.wert()))
            return False
        if not self.m_schalter_1.gueltig():
            log_sup.log().warning("Shelly, {name:16s}, Schalter(1) kein aktueller Wert".format(
                name=self.m_name))
            return False
        if self.m_schalter_1.wert() != "0":
            log_sup.log().debug("Shelly, {name:16s}, Schalter(1) nicht aus, Wert: {wert:s}".format(
                name=self.m_name, wert=self.m_schalter_1.wert()))
            return False
        if not self.m_rollo.gueltig():
            log_sup.log().warning("Shelly, {name:16s}, Rollo kein aktueller Wert".format(
                name=self.m_name))
            return False
        if self.m_rollo.wert() != "stop":
            log_sup.log().debug("Shelly, {name:16s}, Rollo laeuft, Wert: {wert:s}".format(
                name=self.m_name, wert=self.m_rollo.wert()))
            return False
        # final, alle Tests bestanden, return true :-)
        log_sup.log().debug("Shelly, {name:16s}, automatic_mode_oben ok".format(name=self.m_name))
        return True
    ...
    def position(self):
        ...
        if not self.m_rollo_pos.gueltig():
            log_sup.log().warning("Shelly, {name:16s}, Rollo_Pos kein aktueller Wert".format(
                name=self.m_name))
            return False
        log_sup.log().debug("Shelly, {name:16s}, Position Rollo ist {wert:16s}".format(
    ...
    def position_oben(self):
        ...
        if self.m_rollo_pos.wert() != "100":
            log_sup.log().debug("Shelly, {name:16s}, Rollo Position nicht oben, Wert: {wert:s}".format(
                name=self.m_name, wert=self.m_rollo_pos.wert()))
            return False
        # final, alle Tests bestanden, return true :-)
        log_sup.log().debug("Shelly, {name:16s}, Rollo ist oben".format(name=self.m_name))
        return True
    ...
    def schliesse_teilweise(self):
        ...
        """
        if mqtt_sup.publish(self.m_pattern + "/roller/0/command/pos", "40"):
            log_sup.log().info(" Shelly, {name:16s}, Rollo faehrt auf 40%".format(name=self.m_name))
        else:
            log_sup.log().error("Shelly, {name:16s}, mqtt message failed".format(name=self.m_name))

    def schliesse_komplett(self):
        """Shelly:schliesse_komplett
        Gibt das Kommando, den Rollo zu schliessen
        """
        if mqtt_sup.publish(self.m_pattern + "/roller/0/command", "close"):
            log_sup.log().info(" Shelly, {name:16s}, Rollo wird geschlossen".format(name=self.m_name))
        else:
            log_sup.log().error("Shelly,{name:16s}, mqtt message failed".format(name=self.m_name))
...
def on_connect(client, userdata, flags, result):
    ...
    log_sup.log().info(" on_connect, , Connected with result code {r_code:d}".format(
        r_code=result))
    mqtt_sup.subscribe_all()

def on_message(client, userdata, msg):
    """on_message
    MQTT-Callback wenn eine Nachricht ankommt"""
    del client, userdata # nicht benuetzt
    mqtt_sup.update(msg.topic, msg.payload)
...
class mqtt_sup():
    ...
    def subscribe(cls, p):
        ...
        if cls._mqttclient == None:
            cls._mqtt_init()
        return cls._mqttclient.subscribe(p)
    ...
    def publish(cls, p, v):
        ...
        if cls._mqttclient == None:
            cls._mqtt_init()
        return cls._mqttclient.publish(p, v)
